...
fn prune_blocks_on_finalize_and_reorg() {
	...
	let make_block = |index, parent, val: u64| {
		insert_block(&backend, index, parent, None, H256::random(), vec![val.into()], None).unwrap()
	};
...
fn remove_leaf_block_works() {
	...
	for i in 0..2 {
		let hash =
			insert_block(&backend, i, prev_hash, None, Default::default(), vec![i.into()], None)
				.unwrap();
		blocks.push(hash);
		prev_hash = hash;
	}
	...
	let best_hash =
		insert_block(&backend, 1, blocks[0], None, sp_core::H256::random(), vec![42.into()], None)
			.unwrap();
...
fn test_pinned_blocks_on_finalize() {
	...
	// Block tree:
	//   0 -> 1 -> 2 -> 3 -> 4 -> 5
	let hash = insert_block(&backend, 5, prev_hash, None, Default::default(), vec![5.into()], None)
		.unwrap();
	blocks.push(hash);
	...
	let hash = insert_block(&backend, 6, blocks[5], None, Default::default(), vec![6.into()], None)
		.unwrap();
	blocks.push(hash);
...
fn test_pinned_blocks_on_finalize_with_fork() {
	...
	//        \ -> 2 -> 3
	let fork_hash_root =
		insert_block(&backend, 2, blocks[1], None, H256::random(), vec![2.into()], None).unwrap();
