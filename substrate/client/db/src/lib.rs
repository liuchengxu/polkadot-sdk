...
impl<Block: BlockT> BlockImportOperation<Block> {
	...
	// pub fn update_state_backend(&mut self, storage: Storage, state_version: StateVersion) {
	// let root = self.apply_new_state(storage, state_version)?;
	// // 1. old_state + new_keys
	// // 	- Fetch the parent state
	// // 	- Insert new keys to the parent state
	// // 2. recalculate the storage root.
	// }
}
